...
class KeywordsStoppingCriteria(StoppingCriteria):
    ...
    def __call__(self, output_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        ...
        for keyword_id in self.keyword_ids:
            logger.debug(keyword_id.shape)
            if (output_ids[0, -keyword_id.shape[0]:] == keyword_id).all():# BUG:RuntimeError: The size of tensor a (2) must match 
                                                                          # the size of tensor b (3) at non-singleton dimension 0
                return True
    ...
    with gr.Blocks(title="TinyLLaVA", theme=gr.themes.Default(), css=block_css) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=8):
                chatbot = gr.Chatbot(
                    elem_id="chatbot", label="Chatbot", height=550)
                with gr.Row():
                    with gr.Column(scale=8):
                        textbox.render()
