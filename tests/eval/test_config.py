...
from tinyllava.eval.config import build_eval_config, parse_eval_config
...
""",
    ...
    config = parse_eval_config(
        [
            "--config",
            str(config_file),
            "model.model_path=output/override",
            "runtime.num_chunks=4",
            "runtime.chunk_idx=2",
        ]
    )

    assert config.model.model_path == "output/override"
...
def test_benchmark_configs_are_valid(config_file, monkeypatch, tmp_path):
    ...
    assert config.model.model_path == "output/test-model"
