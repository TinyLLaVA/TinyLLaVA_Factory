...
def generate_word_images(
    ...
    for i in range(num_top_words - num, num_top_words):
        ...
        for j, (word_index, color, prob) in enumerate(
            zip(word_indices, colors, probabilities)
        ):
...
def generate_word_images_before(
    ...
    for i in range(num_top_words - num, num_top_words - 1):
        ...
        if not i == num_top_words - num:
            output_ids = (
                top_words_tensor[0, num_top_words - num : i, 0, 0].unsqueeze(0).long()
            )
        ...
        plt.savefig(
            os.path.join(
                output_dir,
                "word_before",
                f"word_image_{i - (num_top_words - num - 1)}.png",
            )
        )
        plt.close()
...
class Monitor:
    ...
    def _register(self, llm_layers_index):
        ...
        mod = self.model
        mod.language_model.model.layers[llm_layers_index].register_forward_hook(
            attention_hook
        )
    ...
    def prepare_input(self):
        ...
        image_tensor = (
            255
            * (images_tensor - images_tensor.min())
            / (images_tensor.max() - images_tensor.min())
        )
        ...
        attention_mask = torch.unsqueeze(
            torch.unsqueeze(
                generate_square_subsequent_mask(length).clone().detach(), dim=0
            ),
            dim=0,
        ).cuda()

        self.hidden = self.model.language_model.model.layers[self.llm_layers_index](
            hidden_tensor, output_attentions=True, attention_mask=attention_mask
        )
        self.image_token = self.image_token[0].squeeze()
        self.image_token = torch.cat(
            (torch.zeros(1, 2560).cuda(), self.image_token), dim=0
        )
    ...
    def get_output(self, output_dir="results/"):
        ...
        for i in range(len(result_top1) - num, len(result_top1)):
            ...
            word_vector = (
                self.model.language_model.model.embed_tokens(word_id_tensor)
                .squeeze()
                .detach()
            )
            ...
            visualize_grid_to_grid(
                "hot_image_" + str(i - (len(result_top1) - num) + 1),
