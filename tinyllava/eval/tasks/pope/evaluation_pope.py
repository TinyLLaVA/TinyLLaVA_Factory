...
def evaluate_pope(
    *,
    ...
    # Generation adapters serialize IDs as strings so heterogeneous benchmark
    # sources (integer/string IDs) share one stable prediction schema.
    questions = {str(row["question_id"]): row for row in read_jsonl(question_file)}
    ...
    for filename in sorted(os.listdir(annotation_dir)):
        ...
        category_predictions = [
            ...
            if questions[str(row["question_id"])]["category"] == category
        ]
