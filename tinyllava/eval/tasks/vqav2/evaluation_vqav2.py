...
from tinyllava.eval.tasks.vqa.vqa_answer import EvalAIAnswerProcessor
...
def convert_vqav2_submission(
    *,
    ...
    prediction_by_id = {
        str(item["question_id"]): item["text"] for item in predictions
    }
    ...
    for row in split_rows:
        ...
        prediction_key = str(question_id)
        if prediction_key not in prediction_by_id:
            missing += 1
            answer = ""
        else:
            answer = answer_processor(prediction_by_id[prediction_key])
